...
const AvailableAssignmentsPage = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(
        err?.response?.data?.error || "Failed to send offer. Try again."
      );
    } finally {
      setSending(false);
    }
    ...
    // Build a full backend URL for the attachment
  const getAttachmentUrl = (assignment: Assignment | null) => {
    if (!assignment?.attachmentPath) return null;

    const path = assignment.attachmentPath;
    ...
    // Default baseURL looks like http://localhost:5000/api -> strip /api
    const match = origin.match(/(.*)\/api\/?$/);
    if (match) {
      origin = match[1];
    }
  ...
  const attachmentUrl = getAttachmentUrl(selected);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500">
          Browse student requests below. Select one to view full details, review
          the attached PDF, and send a tailored offer.
        </p>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {a.budgetMin != null && a.budgetMax != null && (
                    <div className="whitespace-nowrap text-[11px] text-emerald-600">
                      ${a.budgetMin} – ${a.budgetMax}
                    </div>
                  )}
                </div>
      ...
      {/* 2) SEND OFFER SECTION (FULL INFO + FORM + PDF VIEWER) */}
      <div className="rounded-xl bg-white p-4 shadow-sm">
        {selected ? (
          <>
            {/* FULL ASSIGNMENT INFO */}
              ...
              {attachmentUrl && (
                <div className="mt-2">
                  ...
                  <p className="mt-1 text-[11px] text-slate-500">
                    Review the uploaded assignment file before sending your
                    offer.
                  </p>
                  <div className="mt-2 h-64 w-full overflow-hidden rounded border border-slate-200 bg-slate-50">
                    <iframe
                      src={attachmentUrl}
          ...
          <p className="text-xs text-slate-500">
            Select an assignment above to see full details, review the PDF, and
            send an offer.
          </p>
        )}
      </div>
